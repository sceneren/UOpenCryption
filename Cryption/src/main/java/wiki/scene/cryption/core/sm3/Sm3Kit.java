package wiki.scene.cryption.core.sm3;
...
public class Sm3Kit extends AbstractCoder {
    ...
    {
        return back(HexUtils.intToBytes(num));
    }

    private int bigEndianByteToInt(byte[] bytes)
    {
        return HexUtils.byteToInt(back(bytes));
    }
    ...
    public byte[] padding(byte[] in, int bLen)
    {
        ...
        pos += padd.length;
        byte[] tmp = back(HexUtils.longToBytes(n));
    ...
    public String digestSignature(String data, String key) {
        ...
        return HexUtils.byteToHex(sign);
    }
